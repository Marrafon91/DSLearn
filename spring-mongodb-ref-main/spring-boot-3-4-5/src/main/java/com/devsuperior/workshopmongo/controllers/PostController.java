...
public class PostController {
    ...
    public Mono<ResponseEntity<PostDTO>> findById(@PathVariable String id) {
        return service.findById(id).map(postDTO ->
                ResponseEntity.ok().body(postDTO));
    }

    @GetMapping(value = "/titlesearch")
    public Flux<PostDTO> findByText(@RequestParam(
            value = "text", defaultValue = "") String text) throws UnsupportedEncodingException {
    ...
    public Flux<PostDTO> findByUser(@PathVariable("id") String userId) {
